...
export function useIndexes(): ReturnType {
  ...
  const [allIndexes, setAllIndexes] = useState<Index[]>([]); // State to hold index data
   ...
   */
  const fetchIndex = useCallback(async (poolAddress: string) => {
    try {
  ...
  const fetchIndexes = useCallback(async () => {
    const IndexFactoryContract = new NormalIndexFactoryContract.Client({
      contractId: constants.FACTORY_ADDRESS,
      networkPassphrase: constants.NETWORK_PASSPHRASE,
      rpcUrl: constants.RPC_URL,
    });

    const indexes = await IndexFactoryContract.query_indexes({});

    const indexWithData =
      indexes && Array.isArray(indexes.result)
        ? await Promise.all(
            indexes.result.map(async (pool: string) => {
              return await fetchIndex(pool);
            })
          )
        : [];

    const indexesFiltered: Index[] = indexWithData.filter(
      (el: any) =>
        el !== undefined &&
        el.tokens.length >= 2 &&
        el.poolAddress !== 'CBXBKAB6QIRUGTG77OQZHC46BIIPA5WDKIKZKPA2H7Q7CPKQ555W3EVB'
    );
    setAllIndexes(indexesFiltered as Index[]);
    setLoading(false);
  }, [fetchIndex]);
