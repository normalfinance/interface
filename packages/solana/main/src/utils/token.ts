...
export const getTokenAddress = (mintAddress: string, userPubKey: string): Promise<PublicKey> => {
  return getAssociatedTokenAddress(new PublicKey(mintAddress), new PublicKey(userPubKey), true);
};
...
export const getTokenAccount = async (
  ...
  const tokenAccounts = await connection.getParsedTokenAccountsByOwner(new PublicKey(userPubKey), {
...
export const createTokenAccountIx = async (
  ...
  const associatedAddress = await getAssociatedTokenAddress(mintAddress, owner, true);
