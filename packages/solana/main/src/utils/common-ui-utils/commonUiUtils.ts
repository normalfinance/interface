...
import { ENUM_UTILS, sleep } from '..';
import { AccountInfo, Connection, Keypair, ParsedAccountData } from '@solana/web3.js';
 ...
 * @returns
 */
const getIdAndAuthorityFromKey = (key: string): { userId: number; userAuthority: PublicKey } => {
  const splitKey = key?.split('_');

  if (!splitKey || splitKey.length !== 2) return { userId: undefined, userAuthority: undefined };
...
const fetchCurrentSubaccounts = (normalClient: NormalClient): UserAccount[] => {
  return normalClient.getUsers().map((user) => user.getUserAccount());
};

const fetchUserClientsAndAccounts = async (
  normalClient: NormalClient
): Promise<{ user: User; userAccount: UserAccount }[]> => {
  const accounts = fetchCurrentSubaccounts(normalClient);
  const allUsersAndUserAccounts = accounts.map((acct) => {
    return {
      user: normalClient.getUser(acct.subAccountId, acct.authority),
      userAccount: acct,
    };
  });
...
const awaitAccountInitializationChainState = async (
  ...
  const user = normalClient.getUser(userId, authority);
...
/**
 * Using your own callback to do the account initialization, this method will run the initialization step, switch to the normal user, await for the account to be available on chain, subscribe to the user account, and switch to the user account using the normal client.
 *
...
> => {
  await normalClient.addUser(userIdToInit, authority);

  const accountAlreadyExisted = await normalClient.getUser(userIdToInit)?.exists();

  // Do the account initialization step
  let result = await callbacks.initializationStep();

  // Fetch account to make sure it's loaded
  await updateUserAccount(normalClient.getUser(userIdToInit));
  ...
  result = callbacks.postInitializationStep ? await callbacks.postInitializationStep() : result;
  ...
  result = await awaitAccountInitializationChainState(normalClient, userIdToInit, authority);

  if (!result) {
    return 'failed_awaitAccountInitializationChainState';
  }

  await normalClient.switchActiveUser(userIdToInit, authority);
...
async function updateUserAccount(user: User): Promise<void> {
  ...
  try {
    const dataAndContext = await user.normalClient.program.account.user.fetchAndContext(
      publicKey,
      'processed'
    );
...
const compareSignatures = async (original: string, hashed: string): Promise<boolean> => {
...
// const calculateAverageEntryPrice = (quoteAssetAmount: BigNum, baseAssetAmount: BigNum): BigNum => {
...
//     const worstPriceToUse = BN.max(endPriceFromSettings, startPriceFromSettings); // Handles edge cases like if the worst price on the book was better than the oracle price, and the settings are asking to be relative to the oracle price
...
//     const worstPriceToUse = BN.min(endPriceFromSettings, startPriceFromSettings); // Handles edge cases like if the worst price on the book was better than the oracle price, and the settings are asking to be relative to the oracle price
...
//     normalClient.getQuoteValuePerLpShare(marketIndex),
//     QUOTE_PRECISION_EXP
//   );
...
const getTokenAddress = (mintAddress: PublicKey, userPubKey: PublicKey): Promise<PublicKey> => {
...
const getTokenAccount = async (
  ...
  const associatedAddress = await getAssociatedTokenAddress(mintAddress, userPubKey, true);

  const targetAccount =
    tokenAccounts.value.filter((account) => account.pubkey.equals(associatedAddress))[0] ||
    tokenAccounts.value[0];

  const anotherBalanceExists = tokenAccounts.value.find((account) => {
    return (
      !!getBalanceFromTokenAccountResult(account) && !account.pubkey.equals(targetAccount.pubkey)
    );
  });
...
const userExists = async (normalClient: NormalClient, userId: number, authority: PublicKey) => {
...
const formatTokenInputCurried =
  (setAmount: (amount: string) => void, marketConfig: MarketConfig) => (newAmount: string) => {
    ...
    if (lastChar === '0') {
      ...
      if (numOfDigitsAfterDecimal > marketConfig.precisionExp.toNumber()) {
    ...
    const formattedAmount = Number((+newAmount).toFixed(marketConfig.precisionExp.toNumber()));
