...
export const checkIfCountryIsBlocked = async () => {
  ...
  const result = await fetch(`https://geolocation.normalfinance.workers.dev/`, {
    cache: 'no-cache',
  });
  ...
  const countryIsBlocked = !!GEOBLOCK_LIST.find((country) => country.code === country_code);
...
export const useGeoBlocking = (callback?: () => void) => {
  ...
  const onlyGeoBlockMainnet = process.env.NEXT_PUBLIC_ONLY_GEOBLOCK_MAINNET === 'true';

  const ignoreGeoBlock = process.env.NEXT_PUBLIC_IGNORE_GEOBLOCK === 'true' || devSwitchIsOn;
