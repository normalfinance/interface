...
import { OraclePriceStore, useCommonNormalStore, useOraclePriceStore } from '../../stores';
...
export const useSyncOraclePriceStore = (
  ...
  useEffect(() => {
    ...
    ORACLE_CLIENT_KEYS.forEach((oracleSourceKey) => {
      ...
      const oracleClient = getOracleClient(oracleSource, connection);
      setOracleClients((s) => {
        s.set(oracleSourceKey, oracleClient);
        return s;
      });
    });
  }, [connection, normalClient]);
