...
import { getResponseTimes } from '@/utils/rpcLatency';
...
const useRpcLatencies = (rpcOptions: RpcEndpoint[]) => {
  ...
  const [avgLatencies, setAvgLatencies] =
    useState<Record<string, { avg: number; lastFiveLatencies: number[] }>>(initialState);
        ...
        responseTimes.forEach((rpc) => {
          const lastFiveLatencies = !rpc.latency
            ...
            : avgLatencies?.[rpc.value]?.lastFiveLatencies
              ? [rpc.latency, ...(avgLatencies?.[rpc.value]?.lastFiveLatencies || [])].slice(0, 5)
              : [rpc.latency];
...
const useAllRpcLatencies_ = () => {
  const isMainnet = useCommonNormalStore((s) => s.env.normalEnv === 'mainnet-beta');

  const rpcOptions = isMainnet ? EnvironmentConstants.rpcs.mainnet : EnvironmentConstants.rpcs.dev;
