...
export const useSyncPriorityFeeStore = ({
  ...
  const priorityFeeSubscriberRef = usePriorityFeeSubscriber(percentilePriorityFeeStrategy);
  ...
  const updateRecentSubscriberFeeHistory = useCallback(() => {
    ...
    const latestSubscriberFeeResult = priorityFeeSubscriberRef.current.lastCustomStrategyResult;
  ...
  const getDynamicCUPrice = (boostMultiplier?: number) => {
    ...
    const dynamicCUPriceTouse = PriorityFeeCalculator.calculateDynamicCUPriceToUse({
  ...
  const getPriorityFeeToUse = useCallback(
    (computeUnitsLimit?: number, feeTypeOverride?: FeeType) => {
      ...
      if (userPriorityFeeType !== 'custom') {
        const feeCapToComputeUnits = PriorityFeeCalculator.calculateCUPriceForTargetSolValue(
          userCustomMaxPriorityFeeCap,
          computeUnitsLimit
        );
        ...
        const sanityCheckMaxFee = PriorityFeeCalculator.calculateCUPriceForTargetSolValue(
          SANITY_CHECK_ABS_MAX_FEE_IN_SOL,
          computeUnitsLimit
        );
      ...
      const priorityFeeInSol = PriorityFeeCalculator.getPriorityFeeInSolForComputeUnitPrice(
        cuPriceToUse,
        computeUnitsLimit
      );
