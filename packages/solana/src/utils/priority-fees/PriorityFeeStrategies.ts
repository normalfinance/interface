import { PriorityFeeStrategy, SolanaPriorityFeeResponse } from '@normalfinance/solana-sdk';
  ...
  return {
    calculate(newSamples: SolanaPriorityFeeResponse[]) {
      ...
      const filteredSamples = newSamples.filter(
        (sample) => sample.slot > LAST_SEEN_SLOT_IN_SAMPLES && sample.prioritizationFee > 0
      );
      ...
      const targetPercentileIndex = Math.min(
        ...
        Math.ceil((allRecentSamplesAscendingSorted.length / 100) * feeStrategyTargetPercentile)
      );
